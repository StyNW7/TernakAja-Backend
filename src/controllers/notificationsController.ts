...
import { livestockTable, notificationsTable, sensorDataTable } from "../db/schema";
...
export const getRecentNotifications = async (
    ...
    try {
      ...
      const result = await db
        ...
        .innerJoin(livestockTable, eq(notificationsTable.livestockId, livestockTable.id))
  ...
  export const getNotificationsWithLivestockAndSensorData = async (
    ...
    try {
      ...
      const result = await db
        ...
        .from(notificationsTable)
        .innerJoin(livestockTable, eq(notificationsTable.livestockId, livestockTable.id))
        .innerJoin(sensorDataTable, eq(sensorDataTable.livestockId, livestockTable.id))
        .where(eq(notificationsTable.userId, userId));
  
      if (!result.length) {
        res.status(404).json({ error: 'No notifications with associated livestock or sensor data found for this user' });
        return;
      }
  
      res.json({
        message: 'Notifications with livestock and sensor data retrieved successfully',
        data: result,
      });
    } catch (error) {
      console.error('Get notifications with livestock and sensor data error:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  };